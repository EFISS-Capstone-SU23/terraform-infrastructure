resource "google_compute_instance" "mono_minhpvt" {
  name     = "mono-minhpvt-${count.index}"
  count    = 0
  ...
  metadata = {
    ...
    startup-script = "curl -s https://raw.githubusercontent.com/EFISS-Capstone-SU23/k8s-manifest/main/scripts/gcp-scripts/gce-single-node-all-in-one.sh | bash"
  }
